...
- The Contribute tab now has a local uncommitted reading-mode interface in `ContributeScreen.tsx`; live payment confirmation and receipts are not implemented yet. Loans still uses a temporary empty view; Profile is implemented.
- The Profile tab now uses `ProfileScreen.tsx`. It loads/saves full name and phone, locks email and national ID, shows role/verification state, logs out, and uploads profile photos through private Supabase Storage. Password change, receipts, and exports remain deferred.
...
- Development has used temporary manual SQL shortcuts for first-chairman bootstrap, deleting test users, and repairing stuck statuses. Treat those as deferred work, not completed product behavior. Before production, apply and verify migrations 0003-0007 in the live Supabase project and complete registration -> OTP -> pending approval -> chairman approve/reject/remove plus avatar upload without SQL.
